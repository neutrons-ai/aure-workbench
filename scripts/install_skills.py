...
dispatcher agent is generated for each skill in ``.claude/agents/`` and
``.github/agents/`` so it is immediately invocable in the current session.

Usage (run without installing the project):
...
def dispatcher_markdown(skill, rel_skill_md: str) -> str:
...
def install_skill(
    ...
    Returns the list of paths written (or that would be written on a dry run).
    """
    ...
    if dispatchers:
        stub = dispatcher_markdown(skill, rel_skill_md)
...
def cmd_list() -> None:
    ...
    for skill in skills:
        ...
        truncated = desc[:72] + "…" if len(desc) > 72 else desc
        print(f"{skill.name:<{width}}  {truncated}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "-q",
        metavar="TEXT",
        help="Natural-language description of the task to find skills for",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=3,
        metavar="N",
        help="Maximum number of skills to retrieve (default: 3)",
    )
    parser.add_argument(
        "--project-dir",
        type=Path,
    ...
    parser.add_argument(
        "--no-dispatchers",
        action="store_true",
        help="Copy skills into skills/ only; do not generate dispatcher agents",
    )
    parser.add_argument(
        "--agent",
        choices=["claude", "copilot", "both"],
        default=None,
        help=argparse.SUPPRESS,  # deprecated: skills now install to a shared skills/ folder
    )
    parser.add_argument(
        "--list",
        "-l",
    ...
    if args.agent is not None:
        print(
            "warning: --agent is deprecated and ignored; skills now install to the "
            "shared skills/ folder for both Claude Code and Copilot.",
            file=sys.stderr,
        )
