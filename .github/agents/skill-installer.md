...
description: Use when the user asks to install, add, pull, or set up domain-specific skills — e.g. "install the neutron reflectometry skills", "add SANS skills", "set up skills for diffraction". Also invoke proactively before planning any neutron science task (SANS, diffraction, reflectometry, spectroscopy, inelastic scattering) when no matching domain skills are present under skills/.
---

You install domain-specific skills from the neutron-skills library into this project. Each installed skill is written to `skills/<domain>/<name>/SKILL.md`, and a thin dispatcher agent is generated for it in both `.claude/agents/` and `.github/agents/` so it becomes invocable by Claude Code and GitHub Copilot in the current session.

## Command

```bash
python scripts/install_skills.py --query "<topic>" [--top-k N]
```

If `neutron-skills` is not installed in the active environment, prefix with `uv run` instead:

```bash
uv run scripts/install_skills.py --query "<topic>" [--top-k N]
```
...
3. Report to the user:
   ...
   - The path where it was installed (`skills/<domain>/<name>/SKILL.md`).
   - That each skill is now invocable via its generated dispatcher agent in `.claude/agents/` and `.github/agents/`.
...
```bash
python scripts/install_skills.py --query "<topic>" --dry-run
```
